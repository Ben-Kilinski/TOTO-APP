...
export interface LocalizedBlock {
      title: string; // rótulo exibido (ex.: "חומר גלם" | "Ingredientes" | "Ingredients")
      text: string;  // conteúdo do bloco
}
...
export interface Dish {
      ...
      en: string;
      image: string;
      description: LocalizedBlocks; // << agora é array de blocos por idioma
}

// formato antigo (apenas para migração)
interface OldDish {
      he: string;
      pt: string;
      en: string;
      image: string;
      description: { he: string; pt: string; en: string };
}

// --- LABELS (detecção) E TÍTULOS (exibição) ---
const LABELS = {
      he: [
            { key: "ingredients", rx: /^חומרי גלם\s*:?\s*/ },
            { key: "about", rx: /^על המנה\s*:?\s*/ },
            { key: "changes", rx: /^שינויים(?: במנה)?\s*:?\s*/ },
            { key: "allergens", rx: /^רגישויות\s*:?\s*/ },
      ],
      pt: [
            { key: "ingredients", rx: /^Ingredientes\s*:?\s*/ },
            { key: "about", rx: /^(Sobre o prato|Sobre)\s*:?\s*/ },
            { key: "changes", rx: /^(Variações|Alterações)\s*:?\s*/ },
            { key: "allergens", rx: /^(Alergias|Alergênicos)\s*:?\s*/ },
      ],
      en: [
            { key: "ingredients", rx: /^Ingredients\s*:?\s*/ },
            { key: "about", rx: /^(About the dish|About)\s*:?\s*/ },
            { key: "changes", rx: /^(Modifications|Changes)\s*:?\s*/ },
            { key: "allergens", rx: /^Allergens\s*:?\s*/ },
      ],
} as const;

const TITLES = {
      he: { ingredients: "חומר גלם", about: "על המנה", changes: "שינויים", allergens: "רגישויות" },
      pt: { ingredients: "Ingredientes", about: "Sobre o prato", changes: "Variações", allergens: "Alergias" },
      en: { ingredients: "Ingredients", about: "About", changes: "Modifications", allergens: "Allergens" },
} as const;

type SectionKey = keyof typeof TITLES["he"];
...
function splitIntoBlocks(text: string, lang: Lang): LocalizedBlock[] {
      ...
      for (const raw of lines) {
            ...
            const found = LABELS[lang].find(({ rx }) => rx.test(line));
            if (found) {
                  current = found.key as SectionKey;
            ...
            if (!current) current = "about"; // padrão: se não veio rótulo, trata como "sobre"
            if (!buckets.has(current)) buckets.set(current, []);
            buckets.get(current)!.push(line);
      }

      const order: SectionKey[] = ["about", "ingredients", "changes", "allergens"];
      const out: LocalizedBlock[] = [];
      for (const k of order) {
            const arr = buckets.get(k);
            if (arr && arr.length) out.push({ title: TITLES[lang][k], text: arr.join("\n") });
      }
      // fallback: se nada foi detectado, bota tudo como "about"
      return out.length ? out : [{ title: TITLES[lang].about, text }];
}

function convertDish(old: OldDish): Dish {
      return {
            he: old.he,
            pt: old.pt,
            en: old.en,
...
// ---------------------------------------------------------------------------
// DADOS NO FORMATO ANTIGO (OS MESMOS QUE VOCÊ ENVIOU) — APENAS RENOMEADOS
// ---------------------------------------------------------------------------

...
export default function FoodCarousel({ language }: FoodCarouselProps) {
  ...
  return (
    <div className="w-full h-[calc(100vh-60px)] flex items-center justify-center bg-black relative overflow-hidden">
      {/* Botão à esquerda */}
      <button
        onClick={handlePrev}
      ...
      <div className="flex items-center justify-center w-full max-w-screen-xl px-2 relative">
        ...
        {[getIndex(-1), getIndex(0), getIndex(1)].map((dishIndex, i) => {
          ...
          return (
            <div
              key={dishIndex}
              className={`absolute transition-all duration-500 ease-linear flex-shrink-0 rounded-xl border border-zinc-700 bg-zinc-800 text-white shadow-md overflow-hidden 
